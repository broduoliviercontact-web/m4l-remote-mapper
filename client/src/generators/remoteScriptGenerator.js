...
export function generateRemoteScriptFiles({ target, mappings, scriptSlug = createScriptSlug(target.targetDeviceName), customLayouts = [], controlPool = [], layoutStack = [] }) {
  const activeMappings = mappings.filter((mapping) => mapping.source && Number.isFinite(Number(mapping.source.frameworkChannel)) && Number.isFinite(Number(mapping.source.data1)))
  const profile = {
    schemaVersion: '0.1',
    ...
    mappings: mappings.map((mapping) => ({
      ...
      layoutInstanceId: mapping.layoutInstanceId,
      layoutName: mapping.layoutName,
      createdBy: mapping.createdBy,
      visualControlId: mapping.visualControlId,
      visualControlKind: mapping.visualControlKind,
      visualControlLabel: mapping.visualControlLabel,
...
class ${scriptSlug}(ControlSurface):
    ...
    MAPPINGS = [
${buildPythonMappings(activeMappings, target)}    ]
...
- Layout mappings saved in profile: **${mappings.length}**
