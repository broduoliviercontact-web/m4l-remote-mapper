...
export default function AbletonDeviceMapper() {
  ...
  const [customCounts, setCustomCounts] = useState({ faders: 2, knobs: 3, buttons: 1, actions: 1 })
  ...
  const [uiTheme, setUiTheme] = useState(() => {
    const sharedTheme = readSharedTheme('ableton-device-mapper-theme', 'normal')
    return sharedTheme === 'classic' ? 'normal' : sharedTheme
  })
  ...
  useEffect(() => {
    ...
    document.title = uiTheme === 'monotype' ? 'Ableton Device Mapper — Monotype' : uiTheme === 'night' ? 'Ableton Device Mapper — Night' : terminal ? 'Ableton Device Mapper — Terminal Edition' : 'Ableton Device Mapper'
    document.documentElement.dataset.uiTheme = uiTheme
    document.documentElement.classList.remove('theme-terminal', 'theme-classic', 'theme-monotype', 'theme-night')
    document.documentElement.classList.add(`theme-${uiTheme === 'normal' ? 'classic' : uiTheme}`)
  ...
  const fillParameters = () => {
    const result = fillEmptyMappingParameters(mappingsRef.current, selectedDevice)
    setMappings(result.mappings)
    setProfileMessage(result.filledCount ? `Filled ${result.filledCount} empty target${result.filledCount === 1 ? '' : 's'} from the ${selectedDevice.deviceName} catalog.` : 'No compatible empty targets to fill.')
  }
  ...
  return <div className={`app-shell native-mapper ui-theme-${uiTheme} ${terminalTheme ? 'unified-terminal-shell' : nightTheme ? 'monotype-edition night-edition' : monotypeTheme ? 'monotype-edition' : ''}`}>
    <div className="ambient-grid"/>
    {terminalTheme ? <header className="topbar ascii-boot-header terminal-command-bar ascii-terminal"><div className="terminal-header-commands"><nav className="mapper-switcher" aria-label="Mapper type"><a href={`${import.meta.env.BASE_URL}?mapper=m4l`}>[ M4L MAPPER ]</a><a href={`${import.meta.env.BASE_URL}?mapper=ableton`} className="active">[ ABLETON MAPPER ]</a></nav><div className="header-actions terminal-header-actions"><div className="terminal-catalog-readout">LIVE {catalog.abletonVersion} / {catalog.deviceCount} DEVICES</div><ThemeSwitcher theme={uiTheme} onChange={setUiTheme}/></div></div></header> : <header className="topbar"><a className="brand native-brand" href={`${import.meta.env.BASE_URL}?mapper=ableton`} aria-label="Ableton Device Mapper home"><span className="brand__mark"><i/><i/><i/><i/></span><span>Ableton <strong>Device Mapper</strong></span></a><nav className="mapper-switcher" aria-label="Mapper type"><a href={`${import.meta.env.BASE_URL}?mapper=m4l`}>Max for Live Mapper</a><a href={`${import.meta.env.BASE_URL}?mapper=ableton`} className="active">Ableton Device Mapper</a></nav><div className="header-actions"><div className="catalog-status"><span>LIVE {catalog.abletonVersion}</span><Badge>{catalog.deviceCount} DEVICES</Badge></div><ThemeSwitcher theme={uiTheme} onChange={setUiTheme}/></div></header>}
    <main id="top">
      <section className="product-summary" aria-label="Ableton Device Mapper summary">Map MIDI controls to native Ableton devices and export a ready-to-install Remote Script.</section>
...
function ThemeSwitcher({theme,onChange}) { return <div className="theme-switcher" role="group" aria-label="Interface style"><span>THEME:</span><button type="button" className={theme==='terminal'?'active':''} aria-pressed={theme==='terminal'} onClick={()=>onChange('terminal')}>[ TERMINAL ]</button><button type="button" className={theme==='normal'?'active':''} aria-pressed={theme==='normal'} onClick={()=>onChange('normal')}>[ CLASSIC ]</button><button type="button" className={theme==='monotype'?'active':''} aria-pressed={theme==='monotype'} onClick={()=>onChange('monotype')}>[ MONOTYPE ]</button><button type="button" className={theme==='night'?'active':''} aria-pressed={theme==='night'} onClick={()=>onChange('night')}>[ NIGHT ]</button></div> }
