...
export default function AbletonDeviceMapper() {
  ...
  return <div className={`app-shell native-mapper ui-theme-${uiTheme} ${terminalTheme ? 'unified-terminal-shell' : ''}`}>
    ...
    {terminalTheme ? <header className="topbar ascii-boot-header terminal-command-bar ascii-terminal"><div className="terminal-header-commands"><nav className="mapper-switcher" aria-label="Mapper type"><a href={`${import.meta.env.BASE_URL}?mapper=m4l`}>[ M4L MAPPER ]</a><a href={`${import.meta.env.BASE_URL}?mapper=ableton`} className="active">[ ABLETON MAPPER ]</a></nav><div className="header-actions terminal-header-actions"><div className="terminal-catalog-readout">LIVE {catalog.abletonVersion} / {catalog.deviceCount} DEVICES</div><ThemeSwitcher theme={uiTheme} onChange={setUiTheme}/></div></div></header> : <header className="topbar"><a className="brand" href={`${import.meta.env.BASE_URL}?mapper=m4l`} aria-label="M4L Remote Mapper home"><span className="brand__mark"><i/><i/><i/><i/></span><span>M4L <strong>Remote Mapper</strong></span></a><nav className="mapper-switcher" aria-label="Mapper type"><a href={`${import.meta.env.BASE_URL}?mapper=m4l`}>Max for Live Mapper</a><a href={`${import.meta.env.BASE_URL}?mapper=ableton`} className="active">Ableton Device Mapper</a></nav><div className="header-actions"><div className="catalog-status"><span>LIVE {catalog.abletonVersion}</span><Badge>{catalog.deviceCount} DEVICES</Badge></div><ThemeSwitcher theme={uiTheme} onChange={setUiTheme}/></div></header>}
    <main id="top">
